...
//
// Filename: ErrorLoadingDataException.cs
...
namespace WikiquoteScreensaverLib.Common.ErrorHandling
{
    ...
    /// </summary>
    [Serializable]
    public sealed class ErrorLoadingDataException 
        : WikiquoteScreensaverLibException
    {
        readonly string _filename;

        internal ErrorLoadingDataException(string filename, Exception innerException)
            : base(innerException)
        {
            _filename = filename;
        }

        internal ErrorLoadingDataException(string filename)
            : this(filename, null)
        {
        }

        internal ErrorLoadingDataException()
            : base()
        {
        }

        private ErrorLoadingDataException(SerializationInfo info, StreamingContext context)
            : base(info, context)
        {
        }
