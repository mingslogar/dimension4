...
namespace AddressParser
{
	...
	public class USAddressParser
	{
		...
		public USAddressParseResult ParseAddress(string input)
		{
			if (!string.IsNullOrWhiteSpace(input))
			{
				Match match = addressRegex.Match(input.ToUpperInvariant());
				if (match.Success)
				{
					Dictionary<string, string> extracted = GetApplicableFields(match);
					return new USAddressParseResult(Normalize(extracted));
				}
			}
		...
		{
			Dictionary<string, string> applicable = new Dictionary<string, string>();

			foreach (string field in addressRegex.GetGroupNames())
			{
		...
		private static void InitializeRegex()
		{
			...
			string rangedSecondaryUnitPattern =
				@"(?<SECONDARYUNIT>" +
				string.Join("|", rangedSecondaryUnits.Keys) +
				@")(?![a-z])";

			string rangelessSecondaryUnitPattern =
				@"(?<SECONDARYUNIT>" +
				string.Join(
					"|",
			...
			string allSecondaryUnitPattern = string.Format(
				CultureInfo.InvariantCulture,
				@"
                    (
                        (:?
			...
			string cityAndStatePattern = string.Format(
				CultureInfo.InvariantCulture,
				@"
                    (?:
                        (?<CITY>[^\d,]+?)\W+
			...
			string placePattern = string.Format(
				CultureInfo.InvariantCulture,
				@"
                    (?:{0}\W*)?
                    (?:(?<ZIP>{1}))?
		...
		private static Dictionary<string, string> Normalize(Dictionary<string, string> extracted)
		{
			...
			foreach (KeyValuePair<string, string> pair in extracted)
			{
				string key = pair.Key;
