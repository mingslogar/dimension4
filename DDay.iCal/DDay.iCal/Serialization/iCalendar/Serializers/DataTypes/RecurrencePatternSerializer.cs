...
namespace DDay.iCal.Serialization.iCalendar
{
		...
		EncodableDataTypeSerializer
	{
		...
		{
			if (byValue != null && byValue.Count > 0)
			{
		...
		public override string SerializeToString(object obj)
		{
			...
			if (recur != null && factory != null)
			{
				...
				if (recur.Count.HasValue)
					values.Add("COUNT=" + recur.Count.Value);

				if (recur.ByDay != null && recur.ByDay.Count > 0)
				{
		...
		public override object Deserialize(TextReader tr)
		{
			...
			if (r != null && factory != null)
			{
								...
								case "UNTIL":
									{
										...
										if (serializer != null)
										{
											...
											IDateTime dt = serializer.Deserialize(reader) as IDateTime;
											reader.Dispose();
							...
							(match = Regex.Match(item, @"(?<Type>second|minute|hour|day|week|month)\s+(?<Num>\d+)", RegexOptions.IgnoreCase)).Success)
						{
							...
							if (int.TryParse(match.Groups["Num"].Value, out num))
							{
								switch (match.Groups["Type"].Value.ToLower())
								{
									case "second":
										if (r.BySecond == null)
											r.BySecond = new List<int>();

										r.BySecond.Add(num);
										break;
									case "minute":
										if (r.ByMinute == null)
											r.ByMinute = new List<int>();

										r.ByMinute.Add(num);
										break;
									case "hour":
										if (r.ByHour == null)
											r.ByHour = new List<int>();
									...
									case "day":
										switch (r.Frequency)
										{
											case FrequencyType.Yearly:
												if (r.ByYearDay == null)
													r.ByYearDay = new List<int>();

												r.ByYearDay.Add(num);
												break;
											case FrequencyType.Monthly:
												if (r.ByMonthDay == null)
													r.ByMonthDay = new List<int>();
									...
									case "week":
										if (r.ByWeekNo == null)
											r.ByWeekNo = new List<int>();

										r.ByWeekNo.Add(num);
										break;
									case "month":
										if (r.ByMonth == null)
											r.ByMonth = new List<int>();
				...
				if (r != null)
				{
					...
					CheckRange("COUNT", r.Count.HasValue ? r.Count.Value : int.MinValue, 0, int.MaxValue);
