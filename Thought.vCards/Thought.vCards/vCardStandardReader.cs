...
namespace Thought.vCards
{
	...
	public class vCardStandardReader : vCardReader
	{
		...
		/// <summary>
		///     Reads a vCard (VCF) file from an input stream. [Deprecated]
		/// </summary>
		/// <param name="card">
		...
		#region [ ReadInto(List<vCard>, TextReader) ]
		...
		///     The vCard with values updated from the file.
		/// </returns>
		public override void ReadIntoList(List<vCard> cardList, TextReader reader)
		{
		...
		public void ReadInto(vCard card, vCardProperty property)
		{
			...
			switch (property.Name.ToUpperInvariant())
			{
				...
				case "X-MS-IMADDRESS":
					ReadInto_X_MS_IMADDRESS(card, property);
					break;
		...
		#region [ ReadInto_X_MS_IMADDRESS ]
		...
		private void ReadInto_X_MS_IMADDRESS(vCard card, vCardProperty property)
		{
			// The X-MS-IMADDRESS property specifies the IM address
			// of the entity associated with the vCard.

			card.IMAddress = property.ToString();
		}
